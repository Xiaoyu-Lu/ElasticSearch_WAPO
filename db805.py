...
@db.connection_context()
def query_doc_805(doc_id: int) -> Dict[str, Any]:
    """
...
def embolden_text_805(query_text: str, doc_idx: int) -> str:
    ...
    try:
        ...
        # database all, calculate the word_idx_dict from scratch
        doc_dict: Dict[str, Any] = query_doc(doc_idx)
        doc: str = doc_dict["content_str"]
        doc_tokens_basic: List[str] = nltk.word_tokenize(doc)
        word_idx_dict = get_word_dict(doc)
...
if __name__ == "__main__":
    pass
    # comment the following lines out if you want to build the db
