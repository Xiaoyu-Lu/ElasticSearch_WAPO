...
def results():
    ...
    return render_template("results.html", items_list=items, ids_list=response, page_id=2, query_text=query_text,
...
def next_page(page_id):
    query_text = request.form["query"]
    option_analyzer = request.form["option_analyzer"]
    option_embed = request.form["option_embed"]
    ...
    return render_template("results.html", items_list=items, ids_list=response, page_id=page_id + 1,
                           query_text=query_text,
                           hits_num=len(response), start=(page_id - 1) * DOC_PER_PAGE + 1, DOC_PER_PAGE=DOC_PER_PAGE,
                           option_analyzer=option_analyzer, option_embed=option_embed)
